...
/**
 * Attest one browser profile and bind its product editor adapters atomically.
 * The returned pack deliberately exposes no token, codec, decrypt, or policy API.
 */
export async function connectPrivacyPack({
  ...
  if (existing) {
    if (
      ...
      || existing.app !== app
      || !sameAdapterBindings(existing.adapters, adapters)
    ) {
      existing.status = "conflict";
      throw new PrivacyPackConnectionError("browser_binding_conflict");
    }
  ...
  const entry = {
    ...
    fingerprint,
    adapters: { ...adapters },
    requirements: attestation.requiredBrowserAdapters.map((item) => ({
      id: String(item.id),
  ...
  entry.pack = Object.freeze({
    packId: id,
    contract,
    fingerprint,
    readiness: Object.freeze({
      get state() { return entry.status; },
      requireReady() {
  ...
  try {
    ...
    CONNECTED_PRIVACY_PACKS.delete(id);
...
function validateServerAttestation({ id, contract, fingerprint, adapters, attestation }) {
  ...
  if (expectedIds.length !== suppliedIds.length || expectedIds.some((idValue, index) => idValue !== suppliedIds[index])) {
...
function sameAdapterBindings(left, right) {
...
function registerPrivacyLifecycleExtension(app) {
  ...
  app.registerExtension({
    ...
    async beforeRegisterNodeDef(nodeType, nodeData) {
      await reconcilePrivacyNodeDefinition(nodeType, nodeData);
    },
...
async function reconcilePrivacyNode(node, phase) {
  for (const entry of CONNECTED_PRIVACY_PACKS.values()) {
    if (entry.status === "ready") await reconcileEntryNode(entry, node, phase);
  }
}
...
async function reconcilePrivacyNodeDefinition(nodeType, nodeData) {
